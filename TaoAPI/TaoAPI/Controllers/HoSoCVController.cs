    ...
    public class HoSoCVController : ControllerBase
    {
            ...
            [FromBody] CreateHoSoCvRequest request)
        {
            try
            {
                ...
                if (ungVien == null)
                {
                    return NotFound(new
                    {
                        message = "Không tìm th?y ?ng viên theo tài kho?n này"
                    });
                }
        ...
        public async Task<ActionResult> GetAllByTaiKhoan(int idTaiKhoan)
        {
            ...
            catch (Exception ex)
            {
                return StatusCode(500, new
                {
                    message = "L?i khi l?y danh sách CV",
                    error = ex.Message,
                    inner = ex.InnerException?.Message
                });
            }
            ...
            [FromBody] CreateHoSoCvRequest request)
        {
            try
            {
                ...
                if (ungVien == null)
                {
                    return NotFound(new
                    {
                        message = "Không tìm th?y ?ng viên"
                    });
                }
                ...
                return Ok(new
                {
                    message = "C?p nh?t CV thành công",
                    idCv = cv.IdCv,
                    loaiMauCv = cv.LoaiMauCv
                });
            }
            ...
            [FromBody] UpdateCvStatusRequest request)
        {
            try
            {
                ...
                return Ok(new
                {
                    message = "C?p nh?t tr?ng thái CV thành công"
                });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new
                {
                    message = "L?i khi c?p nh?t tr?ng thái CV",
                    error = ex.Message,
                    inner = ex.InnerException?.Message
                });
            }
            ...
            int idTaiKhoan)
        {
                ...
                request.Email != taiKhoan.Email)
            {
                ...
                if (emailDaTonTai)
                {
                    ...
                    {
                        message = "Email này dã du?c tài kho?n khác s? d?ng"
                    });
                }
